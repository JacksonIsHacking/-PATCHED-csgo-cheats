<h1> PATCHED </h1>


<h3> Follow These Steps For The Cheats </h3>

<ul>
  <li>1. Download The Osiris.Dll File</li>
  <li>2. Then If You Dont Have An Injector Install The CSGhost-v4.3.1.exe File</li>
  <h3> DON'T OPEN CSGO YET </h3>
  <li>3. Run CSGhost-v4.3.1.exe It Will Bypass CSGO's Anticheat Then It Will Open CSGO</li>
   <li>4. Then Go Back To The Injector And Hit Inject Then Locate Osiris.Dll And Open It With The Injector</li>
  <li>5. Then Enjoy Hacking ❤️</li>
    <li>If You Want A Config For The Cheats Then Intall Zuhns_leaked_config And Import The Config Into The Cheats (Insert Hides The Cheats btw)</li>
</ul>

![maxresdefault](https://github.com/JacksonIsHacking/csgo-cheats/assets/146037364/3376d8b3-b5f1-4168-a3c9-ffdd8d2b5980)
